...
async function openComments(postId) {
  ...
  content.innerHTML = `
    <div class="modal-title"><a href="${url}" target="_blank" rel="noopener">${
    post.title || '(no title)'
  }</a></div>
    <div class="modal-meta">by ${post.by || '?'} · ${
    post.descendants || 0
  } comments · parent #${postId}</div>
    <div id="cr"></div>`;
...
function makeComment(c, postId) {
  ...
  div.innerHTML = `
    <div class="comment-meta">👤 ${c.by || '[deleted]'} · ${
    c.time ? timeAgo(c.time) : ''
  } · post #${postId}</div>
    <div class="comment-text">${sanitize(
      c.text || '<em>(no text)</em>'
    )}</div>`;
